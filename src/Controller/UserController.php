...
class UserController extends AbstractController
{
    ...
    public function branchUserEdit(UserPasswordHasherInterface $hasher, Branch $branch, ValidatorInterface $validator, UserRepository $UserRepository, Client $client, Request $request, ManagerRegistry $manager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            if ($form->get('password')->getData() !== $form->get('confirmPwd')->getData()) {
                $this->addFlash('alert', 'Les mots de passe ne correspondent pas');
            } elseif ($userExiste && $userExiste->getId() !== $users->getId()) {
                $this->addFlash('alert', 'Cette adresse email est deja utilisé ');
            } else {
                $hachPwd = $hasher->hashPassword($user, $form->get('password')->getData());
                $data->setPassword($hachPwd);
                ...
                $this->addFlash('success', 'L\'utilisateur de la structure a bien été modifié');
                return $this->redirectToRoute('app_user_branch_add', [
                    'id_client' => $client->getId(),
                    'id_branch' => $branch->getId()
                ]);
        ...
        return $this->render('branch/user/user_edit.html.twig', [
            'form' => $form->createView(),
            'client' => $client,
            'errors' => $error,
            'user' => $users,
